...
FreezeKey = "l"
GrabKey = ""
local plr = game:GetService("Players").LocalPlayer local mouse = plr:GetMouse() mouse.KeyDown:connect(function(key) if key == GrabKey then game.Players.LocalPlayer.Character.Humanoid:EquipTool(game.Players.LocalPlayer.Backpack["Dragon Throw"]) game.Players.LocalPlayer.Character["Dragon Throw"]:Activate() game.Workspace.Live[plr.Name]["Dragon Throw"].Activator["Flip"]:Destroy() end end)
function Freeze()
    wait()
    local args = {
...
function ef1()
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "1tsJustCmds";
		Text = "Graab Player and after press L";
	})
end

local Player = game.Players.LocalPlayer
Player.Chatted:connect(function(cht)
	if cht:match("_earthf") then
		ef()
		ef1()
	end
end)
...
function commands()
	...
	print ("_earthf Press L for freeze player only earth {NOT Rejoin} its The best freeze in earth")
